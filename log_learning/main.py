...
def get_config():
    ...
    if config_file.exists():
        ...
        config["DEFAULT"] = {"repo_path": repo_path}
...
def prompt_questions():
    ...
    for question in questions:
        print(f"\n{question}")
        print(
            "(Press Enter twice: once after your answer and once more on an empty line)"
        )
        lines = []
        while True:
            line = input()
...
def create_log_file(repo_path, questions, answers):
    ...
    if os.path.exists(log_file):
        overwrite = input(
            f"Log file for today already exists. Overwrite? (y/n): "
        ).lower()
        if overwrite != "y":
    ...
    try:
        ...
        try:
            ...
            if remotes:
                ...
                print(
                    "To add a remote repository, use: git remote add origin <repository-url>"
                )
